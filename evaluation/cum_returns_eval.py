...
def cumulate_returns2(finance_data, tp_portfolio_data, tn_portfolio_data):
    finance_data = finance_data.copy()
    #
    # portfolio_data = pd.read_csv(modell,sep='\t')
    tp_portfolio_data = tp_portfolio_data.copy()
    tn_portfolio_data = tn_portfolio_data.copy()
...
def cumulate_returns3(finance_data, tp_portfolio_data, tn_portfolio_data):
    finance_data = finance_data.copy()
    #
    # portfolio_data = pd.read_csv(modell,sep='\t')
    tp_portfolio_data = tp_portfolio_data.copy()
    ...
    mean_abweichung_tp = (tp_finance_data['Model_Mean_Long'] - equal_weighted_mean.values).mean()
    mean_abweichung_tn = (tn_finance_data['Model_Mean_Short'] * (-1) - equal_weighted_mean.values).mean()
    mean_abweichung = (
    tp_finance_data['Model_Mean_Long'] + (-1) * tn_finance_data['Model_Mean_Short'] - equal_weighted_mean.values).mean()

    return [mean_abweichung_tp, mean_abweichung_tn, mean_abweichung]