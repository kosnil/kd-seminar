...
startDate = datetime.date(2018, 6, 5)
endDate = datetime.date(2018, 6, 7)
...
columns = ['Timestamp', "ID", "articleCount", "avgSentiment", "stdSentiment", "25quantileSentiment",
           ...
           "nbOfDuplicates"]
# "ibm_articleCount", "sadness_count", "anger_count", "fear_count", "joy_count", "analytical_count", "confident_count", "tentative_count"]
...
while startDate < endDate:
    print("-- Start  prcessing day : ", startDate)
    for company in companies:
        print("- Starting article processing for company :", company)
        # QUERY articles related to current company
        q = ER.QueryArticlesIter(conceptUri=er.getConceptUri(company), lang="eng", dateStart=startDate,
                                 dateEnd=startDate)

        articles = q.execQuery(er, sortBy=["date", "sourceImportance"], sortByAsc=False, lang=["eng"], maxItems=50,
        ...
        sentiment_df = pd.DataFrame(index=range(0, 2000),
                                    columns=pd.date_range(startDate, endDate).format("%Y-%m-%d")[1:])
        sentiment_ibm_df = pd.DataFrame(index=range(0, 7),
                                        columns=pd.date_range(startDate, endDate).format("%Y-%m-%d")[1:])
        ...
        while True:
            ...
            # for startDate in sentiment_df.columns:
        results.iloc[result_index]['Timestamp'] = str(startDate)
        results.iloc[result_index]['ID'] = company
        results.iloc[result_index]['articleCount'] = sentiment_df[str(startDate)].count()
        results.iloc[result_index]['avgSentiment'] = sentiment_df[str(startDate)].mean()
        results.iloc[result_index]['stdSentiment'] = sentiment_df[str(startDate)].std()
        results.iloc[result_index]['25quantileSentiment'] = sentiment_df[str(startDate)].quantile(0.25)
        results.iloc[result_index]['50quantileSentiment'] = sentiment_df[str(startDate)].quantile(0.50)
        results.iloc[result_index]['75quantileSentiment'] = sentiment_df[str(startDate)].quantile(0.75)
        results.iloc[result_index]['maxSentiment'] = sentiment_df[str(startDate)].min()
        results.iloc[result_index]['minSentiment'] = sentiment_df[str(startDate)].max()
        results.iloc[result_index]["socialScore"] = social_df[str(startDate)]
        results.iloc[result_index]['nbOfDuplicates'] = duplicate_df[str(startDate)]
            # ibm
    ...
    print("-- Day fully processed : ", startDate)
    startDate += delta
